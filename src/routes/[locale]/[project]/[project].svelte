<script context="module">
	...
	export async function load({ params }) {
		const data = getOneProject(params.project);
...
<script>
	...
	const loadMore = async (nextPage) => {
		const options = {
			page: nextPage,
			playlistId: projectData.youtube_id
		};
...
<svelte:head>
	<title>{projectData.name} • AcademeG</title>
</svelte:head>
...
<Section classes="overflow-hidden" style="opacity: {-scrollY / 1000 + 1.2};">
	{#if projectData.bg_video}
		...
		<video
			...
			width="100%"
			height="100%"
			poster={projectData.background.url}
			autoplay
			muted
			loop
			webkit-playsinline
			playsinline
		>
			<source src={projectData.bg_video.webm} type="video/webm" />
			<source src={projectData.bg_video.mp4} type="video/mp4" />
		</video>
	{:else}
		<div
			class="absolute w-full h-full bg-no-repeat bg-cover animate"
			style="background-image: url({projectData.background.url}); background-position-x: center; background-position-y: {scrollY / 4}px;"
		/>
	{/if}
	...
	<Container classes="flex flex-col items-center justify-center h-screen text-center z-[2]">
		...
		<H1 classes="max-w-full md:max-w-[90%] animate" data-animate-fly="bottom">{projectData.name}</H1
		>
...
<Section
	...
	style="background-image: url({projectData.background_spec.url})"
>
	<Container classes="pb-15">
		<H3 classes="mb-8 md:mb-10 lg:mb-12 animate" data-animate-fly="top"
			>{$localeStore.specifications}</H3
		>
							...
							{#each videosData.data as item}
								...
								>
									<div class="relative w-full h-44 lg:h-40 xl:h-56">
										...
										<div
											class="absolute z-[1] top-0 left-0 riCht-0 bottom-0 w-full h-full bg-gradient-to-t from-black/50 select-none pointer-events-none group-hover:opacity-0 duration-600"
										/>
										<div
											class="absolute -z-[1] top-0 left-0 riCht-0 bottom-0 w-full h-full bg-no-repeat bg-center bg-cover scale-105 select-none pointer-events-none opacity-0 group-hover:opacity-30 duration-600"
